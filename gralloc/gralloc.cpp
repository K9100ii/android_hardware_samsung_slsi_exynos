    ...
    *stride = ALIGN(w, 16);
    ...
    if (planes == 1) {
        *hnd = new private_handle_t(fd, size, usage, w, h,
        ...
        fd1 = exynos_ion_alloc(ionfd, chroma_size, heap_mask, ion_flags);
        if (fd1 < 0)
        {
            ALOGE("failed to get fd from exynos_ion_alloc, %s, %d\n", __func__, __LINE__);
            close(fd);
        ...
        if (planes == 3) {
            if ((format == HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SP_M_PRIV) ||
                ...
                fd2 = exynos_ion_alloc(ionfd, PRIV_SIZE, ION_HEAP_SYSTEM_MASK, 0);
            } else {
                fd2 = exynos_ion_alloc(ionfd, chroma_size, heap_mask, ion_flags);
            }
            if (fd2 < 0)
            {
